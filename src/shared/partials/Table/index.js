...
const ArrowSort = ({ direction }) => <div className={classNames(style.arrow, direction)} />;
...
Table.HeaderCell = props => {
  ...
  return (
    ...
    >
      <button
        type="button"
        className="text-left font-medium focus:outline-none flex cursor-pointer disabled:cursor-not-allowed"
        onClick={triggerSort}
...
Table.BodyRow = props => {
  ...
  return (
    <div
      className={`${
        props.className || ''
      } text-xs text-gray1 flex items-center flex-row w-full py-2 border-b border-gray3 ${
